...
const TransaksiPage = () => {
  ...
  // Ambil transaksi user saat halaman pertama kali dibuka
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    const newTransaction = {
      ...
      jenis: formData.jenisTransaksi, // biar konsisten sama Dashboard
    };

    const key = `transactions_${currentUser.email}`;
    const userTransactions = JSON.parse(localStorage.getItem(key)) || [];

    const updated = [newTransaction, ...userTransactions];
                    ...
                    <tbody>
                      {transactions.slice(0, 5).map((transaction) => (
                        <tr key={transaction.id} className="table-row">
                            ...
                            <div className="transaction-desc">
                              <div
                                className={`transaction-icon ${
                                  transaction.jenisTransaksi === "pemasukan" ? "icon-pemasukan" : "icon-pengeluaran"
                                }`}
                              >
                                <i
                                  className={`fas ${
                                    transaction.jenisTransaksi === "pemasukan" ? "fa-arrow-down" : "fa-arrow-up"
                                  }`}
                                ></i>
                              </div>
                          ...
                          <td className="table-cell table-date">{formatDate(transaction.tanggal)}</td>
                          <td className="table-cell table-cell-right">
                            <span
                              className={`amount-text ${
                                transaction.jenisTransaksi === "pemasukan" ? "amount-pemasukan" : "amount-pengeluaran"
                              }`}
                            >
